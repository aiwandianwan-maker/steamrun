...
if(-not $SteamRoot -or -not (Test-Path "$SteamRoot\steam.exe")){
    Write-Host "❌ Steam installation directory cannot be found" -ForegroundColor Red
...
try{
    ...
    Write-Host "❌ Download patch package failed" -ForegroundColor Red
    Read-Host "Press Enter to close window"
    exit 1
}
...
# Copy All Patch Files To Steam Folder（静默复制，无逐条输出）
...
# Final Only Prompt（只保留最终结果，无中间详情）
...
if($installComplete){
    ...
    Write-Host "⚠️ Some patch files missing, installation incomplete" -ForegroundColor DarkYellow
}
Start-Sleep -Seconds 2
Start-Process "$SteamRoot\steam.exe"
Read-Host "Press Enter key to close window"
exit 0
